...
const ChatInterface = () => {
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-emerald-100 via-green-50 to-teal-100">
      ...
      {/* Messages Container */}
      <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6 bg-gradient-to-b from-white via-green-50 to-emerald-50">
        {messages.length === 1 ? (
          <div className="flex flex-col items-center justify-center h-full text-center px-8">
            ...
            <div className="max-w-3xl mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                <motion.div 
                  className="bg-gradient-to-br from-green-500 to-emerald-600 p-6 rounded-2xl text-white shadow-xl cursor-pointer hover:shadow-2xl transition-all duration-300"
                  whileHover={{ scale: 1.05 }}
                  onClick={() => setInputMessage("What crops should I plant this season in Kerala?")}
                >
                  <div className="text-3xl mb-3">🌱</div>
                  <h3 className="font-bold text-lg mb-2">Crop Selection</h3>
                  <p className="text-sm opacity-90">Best crops for current season</p>
                </motion.div>
                
                <motion.div 
                  className="bg-gradient-to-br from-blue-500 to-cyan-600 p-6 rounded-2xl text-white shadow-xl cursor-pointer hover:shadow-2xl transition-all duration-300"
                  whileHover={{ scale: 1.05 }}
                  onClick={() => setInputMessage("What are the current weather conditions and farming recommendations?")}
                >
                  <div className="text-3xl mb-3">🌦️</div>
                  <h3 className="font-bold text-lg mb-2">Weather Advice</h3>
                  <p className="text-sm opacity-90">Weather-based farming tips</p>
                </motion.div>
                
                <motion.div 
                  className="bg-gradient-to-br from-purple-500 to-violet-600 p-6 rounded-2xl text-white shadow-xl cursor-pointer hover:shadow-2xl transition-all duration-300"
                  whileHover={{ scale: 1.05 }}
                  onClick={() => setInputMessage("What government schemes are available for farmers in Kerala?")}
                >
                  <div className="text-3xl mb-3">💰</div>
                  <h3 className="font-bold text-lg mb-2">Govt Schemes</h3>
                  <p className="text-sm opacity-90">Subsidies and benefits</p>
                </motion.div>
                
                <motion.div 
                  className="bg-gradient-to-br from-orange-500 to-red-600 p-6 rounded-2xl text-white shadow-xl cursor-pointer hover:shadow-2xl transition-all duration-300"
                  whileHover={{ scale: 1.05 }}
                  onClick={() => setInputMessage("How to identify and treat crop diseases organically?")}
                >
                  <div className="text-3xl mb-3">🐛</div>
                  <h3 className="font-bold text-lg mb-2">Disease Control</h3>
                  <p className="text-sm opacity-90">Pest & disease management</p>
                </motion.div>
                
                <motion.div 
                  className="bg-gradient-to-br from-teal-500 to-green-600 p-6 rounded-2xl text-white shadow-xl cursor-pointer hover:shadow-2xl transition-all duration-300"
                  whileHover={{ scale: 1.05 }}
                  onClick={() => setInputMessage("What are the best organic farming practices for Kerala?")}
                >
                  <div className="text-3xl mb-3">🌿</div>
                ...
                <motion.div 
                  className="bg-gradient-to-br from-indigo-500 to-purple-600 p-6 rounded-2xl text-white shadow-xl cursor-pointer hover:shadow-2xl transition-all duration-300"
                  whileHover={{ scale: 1.05 }}
                  onClick={() => setInputMessage("What are the current market prices for major crops?")}
                >
                  <div className="text-3xl mb-3">📈</div>
                  <h3 className="font-bold text-lg mb-2">Market Prices</h3>
                  <p className="text-sm opacity-90">Live market information</p>
                </motion.div>
              </div>
              
              {/* Sample Questions in Malayalam */}
              <div className="bg-gradient-to-r from-green-100 to-emerald-100 p-6 rounded-2xl border border-green-200">
                <h3 className="text-green-800 font-bold text-lg mb-4">മലയാളത്തിൽ ചോദിക്കാം (Ask in Malayalam):</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                  <button 
                    className="text-left p-3 bg-white rounded-lg border border-green-200 hover:bg-green-50 transition-colors"
                    onClick={() => setInputMessage("നെല്ല് കൃഷിയുടെ ഉത്തമ സമയം എന്നാണ്?")}
                  >
                    "നെല്ല് കൃഷിയുടെ ഉത്തമ സമയം എന്നാണ്?"
                  </button>
                  <button 
                    className="text-left p-3 bg-white rounded-lg border border-green-200 hover:bg-green-50 transition-colors"
                    onClick={() => setInputMessage("വാഴയുടെ ഇല പുള്ളി രോഗം എങ്ങനെ നിയന്ത്രിക്കാം?")}
                  >
